...
const AddJobModal = ({ onClose, onJobAdded }: AddJobModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await jobsApi.create({ company_name: companyName, job_title: jobTitle, job_url: jobUrl || undefined, status, notes: notes || undefined });
      onJobAdded();
    } catch {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100 dark:border-gray-700">
          <h2 className="text-base font-semibold text-gray-900 dark:text-white">Add Job Application</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={1} placeholder="Optional notes..." className={`${inputClass} resize-none`} />
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={loading} className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 disabled:cursor-not-allowed text-white rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2">
              {loading ? (<><svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg>Adding...</>) : 'Add Job'}
            </button>
          </div>
        </form>
      </div>
