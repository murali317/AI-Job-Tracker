    ...
    <div className="min-h-screen flex bg-white dark:bg-gray-900">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-linear-to-br from-indigo-950 via-indigo-800 to-indigo-600 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="relative">
          <h1 className="text-4xl font-bold text-white mb-4 leading-tight">Your job search,<br /><span className="text-indigo-300">organized & supercharged.</span></h1>
          <p className="text-indigo-200 mb-10 leading-relaxed">Join thousands of job seekers who track smarter with AI assistance.</p>
          <div className="space-y-4">
            {['Never lose track of an application', 'AI-powered resume feedback', 'Know your match score before applying'].map((f) => (
        ...
        <p className="text-indigo-400 text-xs relative">Free to start â€” no credit card needed</p>
      </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center px-8 pb-12">
          <div className="w-full max-w-sm">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mb-8">Start tracking your job search today</p>
            {error && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} required placeholder="Jane Smith" className="w-full px-3.5 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm" />
              </div>
              <div>
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="Min. 6 characters" className="w-full px-3.5 py-2.5 pr-10 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm" />
                ...
                {password && (
                  <div className="mt-2">
                    ...
                    <p className={`text-xs mt-1 ${
                      password.length >= 12 && /[A-Z]/.test(password) && /[0-9]/.test(password) && /[^A-Za-z0-9]/.test(password) ? 'text-green-600 dark:text-green-400'
                      : password.length >= 8 && /[A-Z]/.test(password) && /[0-9]/.test(password) ? 'text-blue-600 dark:text-blue-400'
                      : password.length >= 6 ? 'text-amber-600 dark:text-amber-400'
                      : 'text-red-500 dark:text-red-400'
                    }`}>
                      {password.length >= 12 && /[A-Z]/.test(password) && /[0-9]/.test(password) && /[^A-Za-z0-9]/.test(password) ? 'Strong password'
                      : password.length >= 8 && /[A-Z]/.test(password) && /[0-9]/.test(password) ? 'Good password'
                      : password.length >= 6 ? 'Fair — add uppercase & numbers'
            ...
            <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-6">Already have an account?{' '}<Link to="/login" className="text-indigo-600 dark:text-indigo-400 font-medium hover:underline">Sign in</Link></p>
          </div>
        </div>
      </div>
    </div>
